...
set_network() {
    if [[ $1 == "dhcp" ]]; then
        ...
        /home/deepgadget/gadgetini/src/util/GGTN_set_ip.sh $1 $server_ip_address
        return
    else
        if [[ $# -eq 4 ]] || [[ $# -eq 5 ]]; then
            local ip_address=$1
            ...
            /home/deepgadget/gadgetini/src/util/GGTN_set_ip.sh "$server_ip_address" "$ip_address/$cidr" "$gateway" "$dns_primary" "$dns_secondary"
            return
        else
