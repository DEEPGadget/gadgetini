...
| 펌프 4대 Tach 입력 | Pulse CH1~4 | IR 13~16 | `pump_rpm` (4채널 평균) |
...
├── main_loop.py          # 메인 루프 (Polling → 환경 센서 → Controller → Alarm)
│
...
├── alarm_checker.py      # threshold 비교 → SET/DEL 알람 키
├── env_sensors.py        # DHT11/HDC302x 자동 감지 + MPU6050 stub (graceful fallback)
│
...
1. **펌프 RPM 환산 보정** — TIM1 duty 0~100% 스윕하면서 Pulse CH1~4 Hz 측정 → PMP-500의 실제 펄스/회전 결정 → polling 환산식 보정.
...
4. **알람 시뮬레이션** — leak/level DIN 토글, ΔT 임계 초과 시뮬, ambient_temp 임계 초과 등 → `alarm_*` Redis 키 SET/DEL 자동 동작 확인.
5. **통신 fallback** — RS485 케이블 분리 → `comm_consecutive_failures` 카운트 → `comm_timeout` → `comm_disconnected` 단계적 알람 → 재연결 시 자동 복구.
