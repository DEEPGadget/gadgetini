                ...
                if alive:
                    ...
                    if ok:
                        try:
                            ...
                            # reloader.cfg (not the boot cfg) so a hot-reloaded
                            # pump min/max clamp takes effect in manual mode too.
                            control_mode = rd.get(K.CONTROL_MODE) or 'auto'
                            if control_mode == 'manual':
                                pcb_control.apply_manual_pwm(driver, rd, reloader.cfg)
                            else:
