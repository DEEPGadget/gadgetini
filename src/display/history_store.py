...
class HistoryStore:
    ...
    def tick(self):
        ...
        if now - self._last_save >= self.interval_sec * 3:
            self.save()
            self._last_save = now
    ...
    def _flush(self):
        for key, samples in self._accum.items():
            ...
            if key not in self.history:
                self.history[key] = []
            self.history[key].append(round(avg, 2))
