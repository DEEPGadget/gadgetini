...
class XPUTemperatureData(SensorData):
    def __init__(self, title_str, unit_str, min_val, max_val, read_rate=30, max_buffer_size=GRAPH_SIZE, redis=None):
        super().__init__(title_str, unit_str, min_val, max_val, read_rate, max_buffer_size)
        self.redis = redis
    ...
    def read_sensor(self):
        ...
        return self.get_max_temp()

    def get_max_temp(self):
        max_temp = 0.0
        for cur_key in self.keys:
            try:
                temp = float(self.redis.get(cur_key))
                #print(cur_key + " = " + str(temp))
                if temp > max_temp:
                    max_temp = temp
            except:
                continue
...
class XPUPowerData(SensorData):
    ...
    def get_max_power(self):
        ...
        for cur_key in self.keys:
            try:
                power = float(self.redis.get(cur_key))
                #print(cur_key + " = " + str(power))
                if power > max_power:
                    max_power = power
            except:
                continue
...
class CPUTemperatureData(SensorData):
    def __init__(self, title_str, unit_str, min_val, max_val, read_rate=30, max_buffer_size=GRAPH_SIZE, redis=None):
        super().__init__(title_str, unit_str, min_val, max_val, read_rate, max_buffer_size)
        self.redis = redis
...
class CPUUtilData(SensorData):
    ...
    def read_sensor(self):
        ...
        return self.get_max_util()

    def get_max_util(self):
        max_util = 0.0
        for cur_key in self.keys:
            try:
                util = float(self.redis.get(cur_key))
                #print(cur_key + " = " + str(temp))
                if util > max_util:
                    max_util = util
            except:
...
class Chassis_Viewer(Viewer):
    ...
    def draw(self, draw, disp_manager, frame):
        ...
        sub_sensor_data_2 = chassis_humid_data  #sub sensor 1
        sub_sensor_data_1 = chassis_temp_data   #sub sensor 2
...
class XPU_Viewer(Viewer):
    ...
    def draw(self, draw, disp_manager, frame):
        ...
        xpu_temp_data = disp_manager.disp_data[3]
        xpu_power_data = disp_manager.disp_data[4]
        chassis_humid_data = disp_manager.disp_data[1]
        chassis_temp_data = disp_manager.disp_data[2]

        sensor_data = xpu_temp_data     #main sensor
        sub_sensor_data_2 = chassis_humid_data  #sub sensor 1
        sub_sensor_data_1 = xpu_power_data   #sub sensor 2
...
class CPU_Viewer(Viewer):
    ...
    def draw(self, draw, disp_manager, frame):
        ...
        cpu_temp_data = disp_manager.disp_data[5]
        cpu_util_data = disp_manager.disp_data[6]
        chassis_humid_data = disp_manager.disp_data[1]
        chassis_temp_data = disp_manager.disp_data[2]

        sensor_data = cpu_temp_data     #main sensor
        sub_sensor_data_2 = chassis_humid_data  #sub sensor 1
        sub_sensor_data_1 = cpu_util_data   #sub sensor 2
        ...
        if len(sub_sensor_data_1.buffer) > 0:
            draw_aligned_text(draw=draw, text=sub_sensor_data_1.title_str, font_size=6, fill='white', box=(databox_x1, databox_y1+75, (databox_x2-databox_x1)/2-margin, 15), align="center", halign="center", font_path=THIN_FONT_PATH, autoscale=False)
            draw_aligned_text(draw=draw, text=str(f"{round(sub_sensor_data_1.buffer[-1],2):.1f}"), font_size=26, fill='white', box=(databox_x1, databox_y1+90, (databox_x2-databox_x1)/2-margin, 24), align="right", halign="top", font_path=BOLD_FONT_PATH, autoscale=True)
            draw_aligned_text(draw=draw, text=sub_sensor_data_1.unit_str, font_size=15, fill='white', box=(databox_x1+(databox_x2-databox_x1)/2-25, databox_y1+115, 25-2-margin, 15), align="right", halign="top", font_path=FONT_PATH, autoscale=False)
...
class MEM_Viewer(Viewer):
    ...
    def draw(self, draw, disp_manager, frame):
        ...
        mem_util_data = disp_manager.disp_data[7]
        mem_used_data = disp_manager.disp_data[8]
        mem_free_data = disp_manager.disp_data[9]

        sensor_data = mem_util_data     #main sensor
        sub_sensor_data_1 = mem_used_data  #sub sensor 1
        sub_sensor_data_2 = mem_free_data   #sub sensor 2
        ...
        if len(sub_sensor_data_2.buffer) > 0:
            draw_aligned_text(draw=draw, text=sub_sensor_data_2.title_str, font_size=6, fill='white', box=(databox_x1+(databox_x2-databox_x1)/2+margin, databox_y1+75, (databox_x2-databox_x1)/2-margin, 15), align="center", halign="center", font_path=THIN_FONT_PATH, autoscale=False)
            draw_aligned_text(draw=draw, text=str(f"{round(sub_sensor_data_2.buffer[-1],2):.1f}"), font_size=26, fill='white', box=(databox_x1+(databox_x2-databox_x1)/2+margin, databox_y1+90, (databox_x2-databox_x1)/2-margin, 24), align="right", halign="top", font_path=BOLD_FONT_PATH, autoscale=True)
            draw_aligned_text(draw=draw, text=sub_sensor_data_2.unit_str, font_size=15, fill='white', box=(databox_x1+(databox_x2-databox_x1)-25+margin, databox_y1+115, 25-2-margin, 15), align="right", halign="top", font_path=FONT_PATH, autoscale=False)
